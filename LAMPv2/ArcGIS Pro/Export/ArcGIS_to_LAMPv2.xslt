...
<xsl:stylesheet version="1.0" exclude-result-prefixes="esri res t msxsl" 
	...
	<xsl:template name="metadata">
        ...
        <mdb:metadataIdentifier gco:nilReason="missing" />
			</xsl:when>
		</xsl:choose>
		<xsl:for-each select="(/metadata/mdLang/languageCode/@value[. != ''])[1]">
			...
			<xsl:choose>
              ...
              <lan:PT_Locale>
                ...
                <xsl:choose>
                  <xsl:when test="(../../../mdChar/CharSetCd/@value != '')">
                    <xsl:for-each select="(../../../mdChar/CharSetCd/@value[. != ''])[1]">
                      <xsl:variable name="charCode">
                        <xsl:call-template name="charSetCode"/>
                      </xsl:variable>
                      <xsl:variable name="charCodelistID">http://www.isotc211.org/2005/resources/Codelist/gmxCodelists.xml#MD_CharacterSetCode</xsl:variable>
                  ...
                  <xsl:otherwise>
                    <lan:characterEncoding>
                      <xsl:variable name="charCode">utf8</xsl:variable>
                      <xsl:variable name="charCodelistID">http://www.isotc211.org/2005/resources/Codelist/gmxCodelists.xml#MD_CharacterSetCode</xsl:variable>
                      <lan:MD_CharacterSetCode codeList="{$charCodelistID}" codeListValue="{$charCode}">
                        <xsl:value-of select="$charCode"/>
                      </lan:MD_CharacterSetCode>
      ...
      <mdb:defaultLocale gco:nilReason="missing"/>
		</xsl:if>
      ...
      <mdb:parentMetadata>
        ...
        <mdb:metadataStandard>
          <cit:CI_Citation>
            <cit:title>
              <gco:CharacterString>ISO 19115-1:2014 Geographic information -- Metadata -- Part 1: Fundamentals</gco:CharacterString>
            </cit:title>
            <cit:edition>
        ...
        <mdb:metadataProfile>LAMPv2</mdb:metadataProfile>
			</xsl:otherwise>
		</xsl:choose>
      ...
      <mdb:spatialRepresentationInfo>
            ...
            <msr:MD_Georectified>
					...
					<xsl:when test="(/metadata/Esri/ArcGISProfile = 'NAP') and (name() != 'GridSpatRep') and (name() != 'Georect') and (name() != 'Georef') and (/metadata/dataIdInfo[1]/spatRpType/SpatRepTypCd/@value = '002')">
						<msr:MD_GridSpatialRepresentation gco:nilReason="missing"/>
					</xsl:when>
					<xsl:when test="(/metadata/Esri/ArcGISProfile = 'NAP') and (name() != 'VectSpatRep') and (/metadata/dataIdInfo[1]/spatRpType/SpatRepTypCd/@value = '001')">
						<msr:MD_VectorSpatialRepresentation gco:nilReason="missing"/>
					</xsl:when>
				</xsl:choose>
      </mdb:spatialRepresentationInfo>
		</xsl:for-each>
		...
		<xsl:if test="(/metadata/Esri/ArcGISProfile = 'NAP') and (not(/metadata/refSysInfo/RefSystem/refSysID) or (/metadata/refSysInfo/RefSystem/refSysID = '')) and ((/metadata/dataIdInfo[1]/spatRpType/SpatRepTypCd/@value = '001') or (/metadata/dataIdInfo[1]/spatRpType/SpatRepTypCd/@value = '002') or (/metadata/dataIdInfo[1]/spatRpType/SpatRepTypCd/@value = '004'))">
			<mdb:referenceSystemInfo gco:nilReason="missing" />
		</xsl:if>
  ...
  <!-- metadataExtensionInfo not included in ArcGIS metadata -->
	<xsl:template name="identificationInfo">
		...
		<xsl:if test="(/metadata/Esri/ArcGISProfile = 'NAP') and (not(idStatus/ProgCd/@value) or (idStatus/ProgCd/@value = ''))">
			<mri:status gco:nilReason="missing"/>
		</xsl:if>
      ...
      <xsl:for-each select="(idCitation/collTitle[. != ''])[1]">
        <mri:associatedResource>
          <mri:MD_AssociatedResource>
            ...
            <xsl:variable name="code">largerWorkCitation</xsl:variable>
            <xsl:variable name="codelistID">http://www.isotc211.org/2005/resources/Codelist/gmxCodelists.xml#DS_AssociationTypeCode</xsl:variable>
            <mri:associationType>
	...
	<xsl:template name="MD_Usage">
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<mri:userContactInfo gco:nilReason="missing"/>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	...
	<xsl:template name="MD_RangeDimension">
		...
		<xsl:if test="(/metadata/Esri/ArcGISProfile = 'NAP') and (not(seqID) or (seqID = '')) and (not(dimDescrp) or (dimDescrp = ''))">
			<mrc:descriptor gco:nilReason="missing"/>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template name="LI_Source">
      ...
      <mrl:sourceSpatialResolution>
		...
		<xsl:if test="(/metadata/Esri/ArcGISProfile = 'NAP') and ((not(srcDesc) or (srcDesc = '')) and (srcExt != '') and (not(srcCitatn) or (srcCitatn = '')))">
			<mrl:sourceCitation gco:nilReason="missing"/>
		</xsl:if>
    <!--
      ...
      <cit:phone>
        <cit:CI_Telephone>
          ...
          <cit:numberType>
            ...
            <xsl:variable name="codelistID">http://www.isotc211.org/2005/resources/Codelist/gmxCodelists.xml#CI_TelephoneTypeCode</xsl:variable>
	...
	<xsl:template name="CI_Citation">
      ...
      <cit:identifier>
		...
		<xsl:if test="(/metadata/Esri/ArcGISProfile = 'NAP') and (not(citRespParty) or (citRespParty = ''))">
			<cit:citedResponsibleParty gco:nilReason="missing"/>
		</xsl:if>
